...
function App() {
  ...
  const getHighlightedCharSet = (charSet, highlightLocations, color = 'red') => {
    ...
    if (Array.isArray(highlightLocations) && highlightLocations.length > 0 && typeof color === 'string') {
      newCharSet = <></>;

      for (let i = 0; i < charSet.length; i++) {
        const currentChar = charSet[i];
        if (!!!highlightLocations.includes(i + (ttt.getOriginalCharSetLength() - ttt.getCharSet().length))) {
          newCharSet = <>{newCharSet}{currentChar}</>
        } else {
          newCharSet = <>
            {newCharSet}<span style={{ color }}>{currentChar}</span>
          </>
        }
      }
    }
  ...
  return (
    <>
      <input className="" type="number" onChange={generatorTypeChange} value={generatorType}></input>
      <input className="training-characters" type="text" onChange={trainingCharactersChange} value={trainingCharacters}></input>
      ...
      <h1 className={`characters-container ${shakerClass}`}><pre>{getHighlightedCharSet(charSet, ttt.getErrorCharPositions())}</pre></h1>
